...
export default {
    ...
    methods: {
        ...
        submit() {
            this.loading = true;
            if (this.post === undefined) {
                this.form.submit('post', route('admin.posts.store').url(), {
                    headers: {
                    ...
                    .then((result) => {
                        ...
                        window.location = route('admin.posts.index').url()
                    }).catch((err) => {
                    this.loading = false;
                ...
                this.form.submit('post', route('admin.posts.update', this.post.id).url(), {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    },
                    // Transform form data to FormData
